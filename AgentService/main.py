    ...
    try:
        ...
        for idx, segment in enumerate(outline_json["segments"]):
            ...
            if idx == longest_segment_idx:
                ret = await deep_dive_segment(job_id, request.markdown, segment, llm)
...
async def deep_dive_segment(job_id: str, text: str, segment: Dict[str, str], llm):
    job_manager.update_status(
        job_id,
        JobStatus.PROCESSING,
        f"Deep diving into topic: {segment['section']}"
    )
